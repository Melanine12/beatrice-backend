...
CREATE TABLE IF NOT EXISTS `tbl_suivi_maintenances` (
  ...
  KEY `fk_suivi_maintenance_espace` (`espace_id`),
  ...
  CONSTRAINT `fk_suivi_maintenance_espace` FOREIGN KEY (`espace_id`) REFERENCES `tbl_espaces` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `tbl_alertes` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('maintenance_overdue','maintenance_due_soon','maintenance_completed','maintenance_cancelled','maintenance_created','maintenance_updated') NOT NULL,
  `titre` varchar(255) NOT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `fk_alerte_destinataire` (`destinataire_id`),
  KEY `fk_alerte_maintenance` (`maintenance_id`),
  ...
  CONSTRAINT `fk_alerte_destinataire` FOREIGN KEY (`destinataire_id`) REFERENCES `tbl_utilisateurs` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_alerte_maintenance` FOREIGN KEY (`maintenance_id`) REFERENCES `tbl_suivi_maintenances` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Index pour optimiser les requêtes fréquentes
CREATE INDEX `idx_suivi_maintenance_composite` ON `tbl_suivi_maintenances` (`statut`, `date_planifiee`, `priorite`);
CREATE INDEX `idx_alerte_composite` ON `tbl_alertes` (`destinataire_id`, `statut`, `date_creation`);
...
FOR EACH ROW
BEGIN
    IF NEW.statut = 'Lue' AND OLD.statut != 'Lue' AND NEW.date_lecture IS NULL THEN
        SET NEW.date_lecture = NOW();
    END IF;
    
    IF NEW.statut = 'Archivée' AND OLD.statut != 'Archivée' AND NEW.date_archivage IS NULL THEN
        SET NEW.date_archivage = NOW();
    END IF;
END$$
...
LEFT JOIN `tbl_espaces` e ON sm.espace_id = e.id
...
BEGIN
    ...
    read_loop: LOOP
        ...
        IF maintenance_responsable_id IS NOT NULL THEN
            ...
            VALUES (
                ...
                CONCAT('La maintenance "', maintenance_titre, '" est en retard depuis le ', DATE_FORMAT(maintenance_date_planifiee, '%d/%m/%Y')),
                'Urgente',
                maintenance_responsable_id,
                maintenance_id,
                JSON_OBJECT('jours_retard', jours_retard, 'date_planifiee', maintenance_date_planifiee)
            );
        END IF;
    ...
    DECLARE cur_due_soon CURSOR FOR
        ...
        AND id NOT IN (
            SELECT DISTINCT maintenance_id 
            FROM tbl_alertes 
            WHERE type = 'maintenance_due_soon' 
            AND statut NOT IN ('Archivée')
            AND maintenance_id IS NOT NULL
        );
    ...
    read_loop2: LOOP
        ...
        IF maintenance_responsable_id IS NOT NULL THEN
            ...
            VALUES (
                ...
                CONCAT('La maintenance "', maintenance_titre, '" est prévue pour le ', DATE_FORMAT(maintenance_date_planifiee, '%d/%m/%Y')),
                CASE WHEN jours_restants <= 1 THEN 'Urgente' ELSE 'Haute' END,
                maintenance_responsable_id,
                maintenance_id,
                JSON_OBJECT('jours_restants', jours_restants, 'date_planifiee', maintenance_date_planifiee)
            );
        END IF;
