...
-- Trigger pour mettre à jour la date de modification des maintenances
DELIMITER $$
...
-- Trigger pour marquer les alertes comme lues
DELIMITER $$
...
CREATE INDEX `idx_maintenance_priorite` ON `tbl_suivi_maintenances` (`priorite`);
CREATE INDEX `idx_maintenance_type` ON `tbl_suivi_maintenances` (`type`);
CREATE INDEX `idx_alerte_priorite` ON `tbl_alertes` (`priorite`);
CREATE INDEX `idx_alerte_date_lecture` ON `tbl_alertes` (`date_lecture`);
