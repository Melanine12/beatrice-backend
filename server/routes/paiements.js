...
const { Paiement, User } = require('../models');
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = await Paiement.findAll({
      where: whereClause,
        attributes: [
          'statut',
          'type_paiement',
          [Paiement.sequelize.fn('COUNT', Paiement.sequelize.col('id')), 'count'],
        [Paiement.sequelize.fn('SUM', Paiement.sequelize.col('montant')), 'total_montant']
      ],
    ...
    const totalMontant = await Paiement.sum('montant', { where: whereClause });
