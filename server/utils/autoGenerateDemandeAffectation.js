...
async function autoGenerateDemandeAffectationForStockHouseKeeping() {
  try {
    ...
    const stockHousekeepingEntrepot = await Chambre.findOne({
      where: {
        [Op.or]: [
    ...
    if (!stockHousekeepingEntrepot) {
      console.log('❌ Stock House Keeping Entrepot (destination) non trouvé');
      return { success: false, message: 'Stock House Keeping Entrepot (destination) non trouvé' };
    }
